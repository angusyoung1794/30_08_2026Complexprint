...
- Добавлена статья «Как китайский принтер съедает бюджет офиса: скрытые расходы Pantum»
  (slug `kitajskij-printer-sedaet-byudzhet-pantum-skrytye-rashody`). Стоит ПЕРВОЙ (новейшая).
  ВНИМАНИЕ: из-за этого «московская» статья сместилась с 3-й на 4-ю позицию.
- У пользователя пока нет публикации на Dzen → поле dzenUrl НЕ задано. Сделал dzenUrl
  опциональным: в Analytics.jsx (карточка) и AnalyticsArticle.jsx (CTA-блок) ссылка на Дзен
  рендерится только если dzenUrl есть; isBasedOn в JSON-LD отсутствует при undefined.
- Фото сгенерированы (по просьбе пользователя — свои, не из документа): 6 шт. public/images/analytics/pantum-*.jpg.
  Оригиналы из Google Doc можно достать через export?format=docx (проверено на прошлой статье),
  но пользователь просил использовать сгенерированные.
- Таблица Pantum/HP/Kyocera, FAQ (FAQPage JSON-LD), перекрёстные ссылки на статьи
  «рынок Pantum», «тонер», «TCO». Маршрут добавлен в scripts/routes.mjs.
- Проверено: сборка ок, /analytics и статья 200, 6 картинок 200, битых изображений нет.
